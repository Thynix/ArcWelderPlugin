...
$(function () {
                ...
                if (options.container) {
                    if (options.parent) {
                        $elm.parents(options.parent)
                            ...
                            .stop()
                            .slideDown("fast", options.onComplete);
                    } else {
    ...
    ArcWelder.FILE_PROCESSING_OPTIONS = [
        ...
        // {name:"Direct Slicer Uploads", value: ArcWelder.PROCESS_OPTION_SLICER_UPLOADS},
        {
            ...
            value: ArcWelder.PROCESS_OPTION_MANUAL_ONLY,
        },
    ];
    ...
    ArcWelder.CHECK_FIRMWARE_OPTIONS = [
        {
            name: "Automatically When Printer Connects",
            value: ArcWelder.CHECK_FIRMWARE_ON_CONECT,
        },
        {
            name: "Only Check Manually",
            value: ArcWelder.CHECK_FIRMWARE_MANUAL_ONLY,
        },
    ...
    ArcWelder.FirmwareViewModel = function (firmware) {
        ...
        self.supported = ko.observable().extend({
            arc_welder_bool_formatted: self.bool_display_options,
            arc_welder_bool_class: self.bool_class_options,
        });
        ...
        self.warnings = ko.pureComputed(function () {
            ...
            if (success === null) {
                warnings.push(
                    "No firmware check has been completed.  Please make sure your printer is connected, then click 'Check Firmware'.",
                );
            } else if (success) {
                if (type === null) {
                    warnings.push(
                        "Arc welder was unable to identity this firmware.  It might not support arc commands, or it may have bugs, but it may work fine.  Use with caution",
                    );
                } else if (version === null) {
                    warnings.push(
                        "Arc welder was unable to identity firmware version.  It might not support arc commands, or it may have bugs, but it may work fine.  Use with caution",
                    );
                } else {
                    if (supported === false && arcs_enabled) {
                        warnings.push(
                            "Your firmware version indicates that it is not supported, but arcs appear to be supported and enabled.  Use with caution.",
                        );
                    }
                    ...
                    if (g2_g3_z_parameter_supported === null) {
                        ...
                        if (allow_3d_arcs) {
                            warnings.push(
                                "Cannot determine if 3D Arc commands are supported (for use with vase mode), but 3D Arcs are currently enabled in the Arc Welder settings.  Please use with extreme caution!",
                            );
                        } else {
                            warnings.push(
                                "Cannot determine if 3D Arc commands are supported (for use with vase mode).  Since 3D arcs are currently disabled, this should be OK.",
                            );
                        }
                    }
                    if (g2_g3_z_parameter_supported === false && !allow_3d_arcs) {
                        // g2_g3_z_parameter_supported support unknown
                        warnings.push(
                            "3D Arcs are not supported by your firmware.  Since 3D arcs are currently disabled, this should be OK.",
                        );
                    }
        ...
        self.errors = ko.pureComputed(function () {
            ...
            if (!success) {
                if (success !== null) {
                    errors.push(
                        "The last firmware check failed.  Please try again.  Click the help link for troubleshooting tips.",
                    );
                }
            } else {
                if (supported === false && !arcs_enabled) {
                ...
                if (!g90_influences_extruder_setting_correct) {
                    ...
                    if (self.use_octoprint_settings()) {
                        setting_location =
                            "Edit the Ocotprint feature setting 'G90/G91 overrides relative extruder mode'";
                    }

                    var error_string =
                        "Your firmware requires the 'G90/G91 Influences Extruder' setting to be " +
                        correct_setting +
                        ".  " +
                        setting_location +
                        ", set the value to " +
                        correct_setting +
                        ", and run the firmware check again.";
                    errors.push(error_string);
                }
                if (allow_3d_arcs && g2_g3_z_parameter_supported === false) {
                    errors.push(
                        "3D Arcs are enabled, but they are not supported by your firmware.  Edit the Arc Welder settings, uncheck 'Allow 3D Arcs', and run the firmware check again.",
                    );
                }
            }
            return errors;
        ...
        self.g90_influences_extruder_setting = ko.pureComputed(function () {
            ...
            var g90_g91_influences_extruder_octoprint =
                ArcWelder.Tab.octoprint_settings.feature.g90InfluencesExtruder();
            if (self.use_octoprint_settings()) {
        ...
        self.checkFirmware = function () {
            if (self.checking_firmware()) return;
        ...
        self.getFirmwareVersion = function () {
            $.ajax({
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    var message =
                        "Could not retrieve firmware data.  Status: " + textStatus + ".  Error: " + errorThrown;
        ...
        self.checkForFirmwareInfoUpdates = function () {
            if (self.checking_for_firmware_info_updates()) return;
    ...
    ArcWelder.StatisticsViewModel = function (is_progress) {
        ...
        self.space_saved_string = ko.pureComputed(function () {
            return ArcWelder.toFileSizeString(self.space_saved(), 1);
        });

        self.target_file_lines = ko.pureComputed(function () {
            return ArcWelder.toShortNumber(self.lines_processed() - self.points_compressed(), 1);
        });
    ...
    ArcWelder.ArcWelderViewModel = function (parameters) {
        ...
        var initial_firmware_compatibility_visible =
            ArcWelder.getLocalStorage("firmware_compatibility_visible") !== "false";
        ...
        self.firmware_compensation_enabled = ko.pureComputed(function () {
            return (
                self.plugin_settings.firmware_compensation_enabled() &&
        ...
        self.onAfterBinding = function () {
            ...
            self.plugin_settings.path_tolerance_percent.extend({
                arc_welder_numeric: 1,
            });
        };
        ...
        self.onStartupComplete = function () {
            ...
            self.current_files = ko.computed(function () {
                ...
                self.addProcessButtonToFileManager(
                    self.files.listHelper.paginatedItems(),
                ...
                if (!startupComplete && self.files.allItems() && self.files.allItems().length > 0) {
                    ...
                    var stat_data = {
                        ...
                        origin:
                            ArcWelder.getLocalStorage("stat_file_origin") || !self.printer_state.sd()
                                ? "local"
                                : "sdcard",
                    };
            ...
            switch (data.message_type) {
                ...
                case "preprocessing-start":
                    if (self.plugin_settings.notification_settings.show_started_notification()) {
                        ...
                        PNotifyExtensions.displayPopupForKey(options, ArcWelder.PopupKey("preprocessing-start"), [
        ...
        self.getPreprocessingTasks = function () {
            $.ajax({
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    var message =
                        "Could not retrieve preprocessing tasks.  Status: " + textStatus + ".  Error: " + errorThrown;
                    console.error(message);
                    return false;
                },
            });
        ...
        self.addProcessButtonToFileManager = function (current_page, is_printing) {
            ...
            for (var file_index = 0; file_index < current_page.length; file_index++) {
                ...
                if (file.arc_welder) {
                    ...
                    if (file.arc_welder_statistics) {
                        ...
                        if (percent) {
                            ...
                            reduction_string =
                                ": " + (is_down ? "<strong>&darr;</strong>" : "") + percent.toFixed(1) + "%";
                        }
                    }
                    var $weldedInfo = $('<span class="arc-welder info">Arc Welded' + reduction_string + "</span><br/>");
                ...
                if (self.plugin_settings.enabled() || is_welded) {
                    ...
                    var $button = $(
                        '\
                        <div class="btn btn-mini arc-welder' +
                            (disable ? " disabled" : "") +
                            '" title="' +
                            title +
                            '">\
        ...
        self.recursiveFileSearch = function (root, path, origin) {
            ...
            for (var index = 0; index < root.length; index++) {
                var file = root[index];
                if (file.type == "machinecode" && file.path == path && file.origin == origin) return file;
        ...
        self.processButtonClicked = function (event) {
            ...
            var element = self.getFile({
                path: file_data.path,
                origin: file_data.origin,
            });
            ...
            var data = {
                path: encodeURI(file_data.path),
                origin: file_data.origin,
                name: encodeURI(file_data.name),
            };
            $.ajax({
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    var message =
                        "Could not pre-process '" + file_data.path + "'.  Check plugin_arc_welder.log for details.";
                    var options = {
