...
class PreProcessorWorker(threading.Thread):
    ...
    def run(self):
        while True:
            try:
                ...
                path, processor_args, additional_metadata, is_manual_request = self._task_queue.get(False)
                try:
    ...
    def _process(self, path, processor_args, additional_metadata, is_manual_request):
        ...
        logger.info("Calling conversion routine on copied source gcode file to target at %s.", self._source_file_path)
        try:
            ...
            # Log the exception
            logger.exception("An unexpected exception occurred while preprocessing %s.", processor_args["path"])
