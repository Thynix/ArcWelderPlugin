...
from octoprint_arc_welder import firmware_checker, log, preprocessor, utilities
...
class ArcWelderPlugin(
    ...
    ARC_WELDER_GCODE_PARAMETERS = {
        ...
        "EXTRUSION-RATE-VARIANCE-DETECTION-ENABLED": {
            "type": "boolean",
    ...
    def __init__(self):
        ...
        self.settings_default = dict(
            ...
            notification_settings=dict(
                show_queued_notification=True, show_started_notification=False, show_completed_notification=True
            ),
        ...
        if target > current:
            ...
            if current < 2:
                ...
                if self._settings.get(["feature_settings", "select_after_processing"]) in ["both", "auto-only"]:
                    self._settings.set(
                        ["feature_settings", "select_after_processing"], ArcWelderPlugin.PROCESS_OPTION_ALWAYS
                    )
                if self._settings.get(["feature_settings", "print_after_processing"]) in ["both", "auto-only"]:
                    self._settings.set(
                        ["feature_settings", "print_after_processing"], ArcWelderPlugin.PROCESS_OPTION_ALWAYS
                    )
    ...
    def on_settings_save(self, data):
        ...
        if not success:
            error_data = {"message_type": "setting-errors", "data": errors}
    ...
    def check_settings(self, settings):
        ...
        try:
            ...
            firmware_compensation_enabled = bool(settings[key]) if key in settings else self._settings.get([key])

            # check firmware compensation variables.
            key = "mm_per_arc_segment"
            ...
            key = "extrusion_rate_variance_detection_enabled"
            extrusion_rate_variance_detection_enabled = (
                bool(settings[key]) if key in settings else self._settings.get([key])
            )
            ...
            max_gcode_length_detection_enabled = bool(settings[key]) if key in settings else self._settings.get([key])
            ...
            errors.append(
                "An unexpected error occurred while saving your settings.  There was an error converting "
                + key
    ...
    def get_firmware_version_request(self):
        ...
        if self._firmware_checker:
            ...
            if firmware_types_info and "last_checked_date" in firmware_types_info:
                firmware_types_info["last_checked_date"] = utilities.to_local_date_time_string(
    ...
    def check_firmware_response_received(self, result):
        if not result["success"]:
            self.send_notification_toast(
                "error", "Unable To Check Firmware", result["error"], False, "check-firmware", "check-firmware"
            )
        else:
            firmware_types_info = self._firmware_checker.firmware_types_info
            ...
            self.send_firmware_info_updated_message(result["firmware_version"], firmware_types_info)
    ...
    def send_firmware_info_updated_message(self, firmware_info, firmware_types_info):
        ...
        if firmware_info and "last_check_datetime" in firmware_info:
            firmware_info["last_check_datetime"] = utilities.to_local_date_time_string(
    ...
    def _g90_g91_influences_extruder(self):
        if self._use_octoprint_settings:
            g90_g91_influences_extruder = self._settings.global_get(["feature", "g90InfluencesExtruder"])
        else:
            g90_g91_influences_extruder = self._settings.get_boolean(["g90_g91_influences_extruder"])
        if g90_g91_influences_extruder is None:
            g90_g91_influences_extruder = self.settings_default["g90_g91_influences_extruder"]
    ...
    def _overwrite_source_file(self):
        ...
        return overwrite_source_file or (self._target_prefix == "" and self._target_postfix == "")
    ...
    def get_output_file_name_and_path(self, display_name, storage_path, gcode_comment_settings):
        ...
        if not overwrite_source_file:
            ...
            if target_prefix == "" and target_postfix == "":
                ...
                new_name = f"{target_prefix}{file_name}{target_postfix}.{file_extension}"
                new_display_name = f"{target_prefix}{new_display_name}{target_postfix}.{display_extension}"
    ...
    def save_preprocessed_file(self, task, results, additional_metadata):
        ...
        if source_path == target_path:
            ...
            try:
                ...
                target_directory, target_name, target_display_name = self._get_collision_free_filepath(
                    target_path, target_display_name
                )
        ...
        self._file_manager.set_additional_metadata(
            FileDestinations.LOCAL, target_path, "arc_welder_statistics", metadata, overwrite=True, merge=False
        )
        ...
        for key, value in additional_metadata.items():
            if value is not None:
                self._file_manager.set_additional_metadata(
                    FileDestinations.LOCAL, target_path, key, value, overwrite=True, merge=False
                )
    ...
    def _get_collision_free_filepath(self, path, display_name):
        ...
        while self._file_manager.file_exists(
            FileDestinations.LOCAL,
            self._file_manager.join_path(FileDestinations.LOCAL, directory, f"{filename_no_extension}.{extension}"),
        ):
            file_number += 1
            filename_no_extension = f"{original_filename}_{file_number}"
            display_name_no_extension = f"{original_display_name}_{file_number}"

        return directory, f"{filename_no_extension}.{extension}", f"{display_name_no_extension}.{extension}"
    ...
    def preprocessing_started(self, task):
        ...
        data = {
            ...
            "message": f"Preprocessing started for {source_name}",
            "task": task,
        }
        self._plugin_manager.send_plugin_message(self._identifier, data)
        self.send_preprocessing_tasks_update()
    ...
    def preprocessing_progress(self, progress, current_task):
        ...
        # add the source and target name as well as the message type from the task
    ...
    def preprocessing_cancelled(self, task, auto_cancelled):
        if task is not None:
            ...
            if auto_cancelled:
                message = (
                    "Cannot process while printing.  Arc Welding has been cancelled for '{0}'.  The file will be "
            ...
            data = {"message_type": "all-preprocessing-cancelled", "message": message}
        self._plugin_manager.send_plugin_message(self._identifier, data)
        self.send_preprocessing_tasks_update()
    ...
    def preprocessing_success(self, task, results):
        ...
        try:
            metadata = self.save_preprocessed_file(task, results, additional_metadata)
        except TargetFileSaveError:
            data = {
        ...
        data = {"message_type": "preprocessing-success", "arc_welder_statistics": metadata, "task": task}
        self._plugin_manager.send_plugin_message(self._identifier, data)
        ...
        if task["octoprint_args"].get("cant_overwrite", False):
            self.send_notification_toast(
                ...
                "The source file is in use and cannot be overwritten.  The target file was renamed to '{}'.".format(
        ...
        if event == Events.PRINT_STARTED:
            ...
            if not current_process_cancelled and print_after_process_stopped:
                self.send_notification_toast(
                    ...
                    "is completed.",
                    True,
                    key="auto_print_cancelled",
                    close_keys=["auto_print_cancelled"],
                )
    ...
    def get_preprocessor_task(self, source_name, source_path, gcode_comment_settings, is_manual_request, metadata):
        ...
        if not self._get_is_printing():
            ...
            # now override if the WELD parameter is supplied within the gcode
        ...
        allow_3d_arcs = gcode_comment_settings.get("allow_3d_arcs", self._allow_3d_arcs)

        allow_travel_arcs = gcode_comment_settings.get("allow_travel_arcs", self._allow_travel_arcs)

        g90_g91_influences_extruder = gcode_comment_settings.get(
            "g90_g91_influences_extruder", self._g90_g91_influences_extruder
        )

        allow_dynamic_precision = gcode_comment_settings.get("allow_dynamic_precision", self._allow_dynamic_precision)

        default_xyz_precision = gcode_comment_settings.get("default_xyz_precision", self._default_xyz_precision)

        if not default_xyz_precision:
            logger.warning("The default XYZ precision was not specified.  Setting to the default value.")
            default_xyz_precision = self.settings_default.get("default_xyz_precision")
        elif default_xyz_precision < 3:
            logger.warning("The default XYZ precision is less than 3.  Setting to the default value.")
            default_xyz_precision = self.settings_default.get("default_xyz_precision")
        elif default_xyz_precision > 6:
            logger.warning("The default XYZ precision is greater than 6, setting to 6.")
            default_xyz_precision = 6

        default_e_precision = gcode_comment_settings.get("default_e_precision", self._default_e_precision)
        if not default_e_precision:
            logger.warning("The default E precision was not specified.  Setting to the default value.")
            default_e_precision = 5
        elif default_e_precision < 3:
            logger.warning("The default E precision is less than 3.  Setting to the default value.")
            default_e_precision = 3
        elif default_e_precision > 6:
            logger.warning("The default E precision is greater than 6, setting to 6.")
        ...
        max_gcode_length = 0
        if max_gcode_length_detection_enabled:
            max_gcode_length = gcode_comment_settings.get("max_gcode_length", self._max_gcode_length)
        ...
        target_name, target_path, target_display_name = self.get_output_file_name_and_path(
            source_name, source_path, gcode_comment_settings
        )
        return {
            "octoprint_args": {
                "source_name": source_name,
            ...
            "processor_args": {
                ...
                "extrusion_rate_variance_percent": extrusion_rate_variance_percent
                / 100.0,  # Convert to decimal percent
    ...
    def add_file_to_preprocessor_queue(self, source_name, source_path, source_target, is_manual_request):
        ...
        if not results["success"]:
            self.send_notification_toast(
                "warning",
                "Arc-Welder: Unable To Queue File",
                results["error_message"],
                True,
                key="unable_to_queue",
                close_keys=["unable_to_queue"],
            )
            return False
        ...
        if self._show_queued_notification:
            ...
            if self._printer.is_printing():
                message += " Processing will occur after the current print is completed.  The 'Print After Processing' option will be ignored."

            data = {"message_type": "task-queued", "message": message}
            self._plugin_manager.send_plugin_message(self._identifier, data)
